...
$newAzureADApplicationSplat = @{
    ...
    ReplyUrls = @("urn:ietf:wg:oauth:2.0:oob","https://localhost","http://localhost","http://localhost:8400")
}

$app = New-AzureADApplication @newAzureADApplicationSplat
...
$newPartnerAccessTokenSplat = @{
    ...
    Tenant = $($spn.AppOwnerTenantID)
    UseAuthorizationCode = $true
}

$token = New-PartnerAccessToken @newPartnerAccessTokenSplat


write-host "Please approve Exchange consent form." -ForegroundColor Green


$newPartnerAccessTokenSplat_Exchangetoken = @{
...
$Exchangetoken = New-PartnerAccessToken @newPartnerAccessTokenSplat_Exchangetoken
